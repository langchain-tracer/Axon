...
const IntegratedDashboardContent = () => {
  ...
  // NEW: keep the id so we can rehydrate after nodes refresh
  const [selectedNodeId, setSelectedNodeId] = useState<string | null>(null); // NEW
  ...
  // CHANGED: remove noisy console.log; keep one concise if needed
  // console.log(selectedNode, 'selectedNode/n/n/n/n');
  ...
  const fetchTraceData = async (traceId: string) => {
    ...
    try {
      ...
      const calculateCost = (node: any): number => {
        const baseCost = 0.0001;
        const latencyMultiplier = (node.latency || 0) / 1000;
        switch (node.type) {
          case 'llm_start':
          case 'llm_end':
            return baseCost * latencyMultiplier * 10;
          case 'tool_start':
          case 'tool_end':
            return baseCost * latencyMultiplier;
          default:
      ...
      const flowNodes: Node[] = data.nodes.map((node: any, index: number) => ({
        ...
        data: {
          ...node, // keep id inside data too (so .data.id exists)
          label: generateStepName(node),
          type: node.type,
  ...
  // NEW: when nodes refresh, re-find the currently selected node by id
  useEffect(() => {
    if (!selectedNodeId) return;
    const refreshed = nodes.find((n) => n.id === selectedNodeId) || null;
    setSelectedNode(refreshed);
  }, [nodes, selectedNodeId]); // NEW
  ...
  const handleCloseReplay = useCallback(() => {
    setShowReplayInterface(false);
    setReplayResult(null);
    // keep selectedNodeId so re-opening still works; only clear the node object
    setSelectedNode(null); // CHANGED: do NOT clear selectedNodeId
  }, []);
  ...
  // NEW: normalized trace shape for ReplayInterface
  ...
  // Helper: small safe view of selectedNode for ReplayInterface
  const selectedNodeForReplay = useMemo(() => {
    if (!selectedNode?.data) return null;
    // ensure id exists inside the object we pass
    return {
      id: selectedNode.data.id ?? selectedNode.id, // important for ReplayEngine
      ...selectedNode.data,
    };
  }, [selectedNode]); // NEW
      ...
      <div className='flex-1 flex flex-col'>
              ...
              {viewMode === 'cost' && (
                <CostView
                  ...
                  onNodeSelect={(node) => {
                    ...
                    if (reactFlowNode) {
                      setSelectedNode(reactFlowNode);
                      setSelectedNodeId(reactFlowNode.id); // NEW
                      setShowReplaySidebar(true);
                    }
                  }}
                  onShowProjections={() => setViewMode('projections')}
        ...
        {showReplayInterface && selectedNodeForReplay && traceData && (
          <div className='fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4'>
            <div className='w-full max-w-4xl max-h-[90vh] overflow-auto'>
              <ReplayInterface
                selectedNode={selectedNodeForReplay} // CHANGED: stable, includes id
                trace={normalizedTraceForReplay} // NEW: normalized
                onReplayComplete={handleReplayComplete}
                onClose={handleCloseReplay}
              />
            </div>
